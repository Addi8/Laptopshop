...
class BinarySearch:
    def createBinarySearchQuery(self, fieldNameToValueDict):
        ...
        if (
            "hardDriveSize" in fieldNameToValueDict
            and len(fieldNameToValueDict["hardDriveSize"].keys()) > 1
        ):

            if (
                "hardDriveType" not in fieldNameToValueDict
                or "hybrid"
                in fieldNameToValueDict["hardDriveType"]["hardDriveTypeValue"]
            ):
                hardDriveSize_value = fieldNameToValueDict["hardDriveSize"]
                del fieldNameToValueDict["hardDriveSize"]
                fieldNameToValueDict.update(
                    {"ssdSize": hardDriveSize_value, "hddSize": hardDriveSize_value}
                )
        ...
        if boolean_ssd_and_hdd:
            ...
            if "hardDriveSizeRange" in fieldNameToValueDict["hddSize"]:
                ...
                for range in fieldNameToValueDict["hddSize"]["hardDriveSizeRange"]:
                    ...
                    if "minValue" in range and "maxValue" in range:
                        ...
                        hdd_ranges.append(
                            {"range": {"hddSize": {"lte": maxValue, "gte": minValue}}}
                        )
                        hdd_ranges.append(
                            {"range": {"ssdSize": {"lte": maxValue, "gte": minValue}}}
                        )
            ...
            if "hardDriveSizeRange" in fieldNameToValueDict[wanted_hdd_type]:
                ...
                for range in fieldNameToValueDict[wanted_hdd_type][
                    "hardDriveSizeRange"
                ]:
                    # ranged value with ssd and hdd
                    if "minValue" in range and "maxValue" in range:
                        ...
                        minValue = range["minValue"]
                        maxValue = range["maxValue"]
                        hardDriveSizeValues.append(
                            {
                                "range": {
                                    wanted_hdd_type: {"lte": maxValue, "gte": minValue}
                                }
                            }
                        )
                        ...
                        minValue = range["minValue"]
                        hardDriveSizeValues.append(
                            {"range": {wanted_hdd_type: {"gte": minValue}}}
                        )

                    elif "maxValue" in range:
                        maxValue = range["maxValue"]
                        hardDriveSizeValues.append(
                            {"range": {wanted_hdd_type: {"lte": maxValue}}}
                        )

        for fieldName in fieldNameToValueDict:

            if (
                not fieldNameToValueDict[fieldName]
                or fieldName == "ssdSize"
                or fieldName == "hddSize"
            ):
                continue
            ...
            if range_field_name in fieldNameToValueDict[fieldName]:
                ...
                for range in fieldNameToValueDict[fieldName][range_field_name]:
                    if "minValue" in range and "maxValue" in range:
                        ...
                        maxValue = range["maxValue"]
                        field_ranges.append(
                            {"range": {fieldName: {"lte": maxValue, "gte": minValue}}}
                        )
            ...
            # In case of multiple values for the same field, example : ram : [2,4,6], ram is either 2, 4 or 6.
                ...
                if type(fieldNameToValueDict[fieldName][value_field_name]) is list:
                    if (
                        type(fieldNameToValueDict[fieldName][value_field_name][0])
                        is str
                    ):
                    ...
                    sameFieldMultpleValues.append(
                        {
                            "terms": {
                                fieldName: fieldNameToValueDict[fieldName][
                                    value_field_name
                                ]
                            }
                        }
                ...
                # A normal numerical match, example : ram : 8, ram is 8
                    ...
                    type(fieldNameToValueDict[fieldName][value_field_name]) is int
                    or type(fieldNameToValueDict[fieldName][value_field_name]) is float
                ):
                    terms.append(
                        {
                            "term": {
                ...
                ranges.append(
                    {
                        "range": {
                            fieldName: {
                                es_intent: fieldNameToValueDict[fieldName]["value"]
                            }
                        }
                    }
                )
