            ...
            new_interval = [
                {
                    ...
                    "maxValue": interval_list[i + 1]["maxValue"],
                    "counter": counter,
                }
            ]
...
class Backend_Helper:
    ...
    def clean_frontend_json(json_dict):
        ...
        for field in json_dict:
            ...
            if field_range_name in json_dict[field]:
                if (
                    len(json_dict[field][field_range_name]) > 0
                ):  # json_dict ['price']['priceRange']
                    ...
                    if len(json_dict[field].get(field_range_name)) > 1:
                        json_dict[field][field_range_name] = alter_range(
                            json_dict[field][field_range_name]
                        )
    ...
    def clean_for_alexa(json_dict):
        ...
        intent_variable_value = json_dict[json_dict["intentVariable"]][
            json_dict["intentVariable"] + "Value"
        ]
    ...
    def refineReviews(docs):
      ...
      for hit in docs["hits"]["hits"]:
          ...
          for field in hit["_source"]:
              if field == 'asin':
                  asin = hit["_source"][field]
                  continue
              item.update({field: hit["_source"][field]})
          outputProducts[asin] = item
      return outputProducts
