...
class VagueSearchPrice:
    ...
    def computeVaguePrice(self, allDocs, weight, minPrice, maxPrice, counter):
       ...
       :returns A list of Tuples, one for each relevant document. Tuples:  (asin number,  Membership function score)"""
        ...
        trapmf = fuzz.trapmf(
            allPrices, [lowerSupport, float(minPrice), float(maxPrice), upperSupport]
        )
        ...
        res = self.es.search(index="laptops", body=body, size=10000)
            ...
            result.append(
                [
                    ...
                    * fuzz.interp_membership(
                        allPrices, trapmf, float(hit["_source"]["price"])
                    ),
                ]
            )
    ...
    the asin score and membership function score."""
        ...
        for i in range(len(interval_list)):
            ...
            if minPrice == 0:
                lowerSupport = 0
            query.append(
                {"range": {"price": {"gte": lowerSupport, "lte": upperSupport}}},
            )
            trapmf = fuzz.trapmf(
                allPrices,
                [lowerSupport, float(minPrice), float(maxPrice), upperSupport],
            )
        ...
        for hit in res["hits"]["hits"]:
            ...
            for score in fuzzy_logic_results:
                score1 = fuzz.interp_membership(
                    allPrices, score, float(hit["_source"]["price"])
                )
                scores.append(score1)
            result.append([hit["_source"]["asin"], weight * max(scores)])
    ...
    def computeVaguePrice_alternative(
        self, allDocs, clean_data, price_searcher, res_search
    ):
            ...
            if len(clean_data["range"]["price"]["range"]) == 1:
                for range in clean_data["range"]["price"]["range"]:
                    if "minValue" in range and "maxValue" in range:
                        ...
                        if (
                            "counter" in range
                        ):  # multiple intervals merged into one large interval=> counter>1
                            counter = range["counter"]
                            res_search.append(
                                price_searcher.computeVaguePrice(
                                    allDocs, price_weight, min_value, max_value, counter
                                )
                            )
                        else:
                            res_search.append(
                                price_searcher.computeVaguePrice(
                                    allDocs, price_weight, min_value, max_value, 1
                                )
                            )

            else:  # multiple intervals
                res_search.append(
                    price_searcher.computeVaguePriceMultiple(
                        allDocs, price_weight, clean_data["range"]["price"]["range"]
                    )
                )
