...
class VagueHardDrive:
    ...
    def computeVagueHardDrive(self, allDocs, weight, minValue, maxValue, counter):
        ...
        vagueFunction = fuzz.trapmf(
            allHardDrives,
            [lowerSupport, float(minValue), float(maxValue), upperSupport],
        )
            ...
            "query": {
                "bool": {
                    "should": [
                        {
                            "range": {
                                "hddSize": {"gte": lowerSupport, "lte": upperSupport}
                            }
                        },
                        {
                            "range": {
                                "ssdSize": {"gte": lowerSupport, "lte": upperSupport}
                            }
                        },
                    ]
                }
        ...
        res = self.es.search(index="laptops", body=body)
        ...
        for hit in res["hits"]["hits"]:
            ...
            if hit["_source"]["hddSize"] and hit["_source"]["ssdSize"]:
                if hit["_source"]["hddSize"] != 0 and hit["_source"]["ssdSize"] != 0:
                    result.append(
                        [
                            hit["_source"]["asin"],  # hit['_source']['hardDrive'],
                            weight
                            * max(
        ...
        result = list(
            map(tuple, result)
        )  # turn list of list pairs into list of tuple pairs containting (ASIN, score) pairs
        return result
    ...
    def computeVagueHardDrive_multiple(self, allDocs, weight, interval_list, counter):
        ...
        for i in range(len(interval_list)):
            ...
            query.append(
                {"range": {"hddSize": {"gte": lowerSupport, "lte": upperSupport}}},
            )
            query.append(
                {"range": {"ssdSize": {"gte": lowerSupport, "lte": upperSupport}}},
            )
        ...
        print("body ", body)
        res = self.es.search(index="laptop", body=body, size=10000)
        ...
        for hit in res["hits"]["hits"]:
            ...
            # laptop has only hdd Drive
                ...
                scores.append(
                    weight
                    * fuzz.interp_membership(
                        allHardDrives, vagueFunction, float(hit["_source"]["hddSize"])
                    )
                )
            ...
            # laptop has only ssd Drive
                ...
                scores.append(
                    weight
                    * fuzz.interp_membership(
                        allHardDrives, vagueFunction, float(hit["_source"]["ssdSize"])
                    )
                )
    ...
    def computeVagueHardDrive_alternative(
        self, allDocs, clean_data, harddrive_searcher, res_search
    ):
        ...
        if "range" in clean_data["range"]["hardDriveSize"]:
            ...
            for range in clean_data["range"]["hardDriveSize"]["range"]:
                if "minValue" in range and "maxValue" in range:
                    ...
                    if "counter" in range:
                        res_search.append(
                            harddrive_searcher.computeVagueHardDrive(
                                allDocs,
                                hd_size_weight,
                                min_value,
                                max_value,
                        ...
                        res_search.append(
                            harddrive_searcher.computeVagueHardDrive(
                                allDocs, hd_size_weight, min_value, max_value, 1
                            )
                        )  # Discrete value needed not a range
            return res_search
        # else:
