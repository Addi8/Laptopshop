...
class VagueSearchRange:
    ...
    def compute_vague_range_mult_intervals(
        self, allDocs, fieldName, weight, interval_list
    ):
    ...
    :returns A list of Tuples, one for each relevant document. Tuples:  (asin number,  Membership function score)"""
        ...
        for i in range(len(interval_list)):
            ...
            if min == 0:
                lowerSupport = 0
            query.append(
                {"range": {fieldName: {"gte": lowerSupport, "lte": upperSupport}}},
            )
            trapmf = fuzz.trapmf(
                allValues, [lowerSupport, float(min), float(max), upperSupport]
            )
        ...
        for hit in res["hits"]["hits"]:
            ...
            for score in fuzzy_logic_results:
                score1 = fuzz.interp_membership(
                    allValues, score, float(hit["_source"][fieldName])
                )
    ...
    def compute_vague_range(
        self, allDocs, fieldName, weight, minValue, maxValue, counter
    ):
    ...
    :returns A list of Tuples, one for each relevant document. Tuples:  (asin number,  Membership function score)"""
        ...
        trapmf = fuzz.trapmf(
            allValues, [lowerSupport, float(minValue), float(maxValue), upperSupport]
        )
