...
def do_query(data):
    ...
    for field in data.keys():
        if data[field]["weight"] == 5:
            ...
            binary_clean_data[field] = data[
                field
            ]  # weigth doesn't matter for boolean search
        elif (
            data[field]["weight"] == 6
        ):  # This is for alexa_search, will be used at the end
    ...
    if len(binary_clean_data) > 0:
        ...
        res = es.search(index="laptops", body=query)
    ...
    if len(alexa_clean_data) > 0:
        ...
        alexa_result = get_alexa_search_result(
            allDocs, alexa_clean_data, alexa_searcher
        )
    ...
    for field_type in field_value_dict.keys():
        for field_name in field_value_dict[field_type]:
            ...
            if (
                field_weight != 5
            ):  ##Shouldn't happen though because they have already been removed from clean_data
    ...
    if "hardDriveSize" in clean_data and len(clean_data["hardDriveSize"]) > 1:
        ...
        #                                                                                       harddrive_searcher,
        #                                                                                       res_search)
        res_search = harddrive_searcher.computeVagueHardDrive_alternative(
            allDocs, field_value_dict, harddrive_searcher, res_search
        )
    #  --------------------------------------------------------------------#
    ...
    if "price" in clean_data and len(clean_data["price"]) > 1:
        ...
        res_search = price_searcher.computeVaguePrice_alternative(
            allDocs, field_value_dict, price_searcher, res_search
        )

    # --------------------------------------------------------------------#
    # Gets scores for all other attributes
    res_search += call_responsible_methods(
        allDocs,
        field_value_dict,
        range_searcher,
        binary_searcher,
    ...
    outputProducts = getElementsByAsin(
        asinKeys
    )  # calls helper class method refineResuls
    ...
    outputProducts = sorted(
        outputProducts, key=lambda x: x["vaguenessScore"], reverse=True
    )
    ...
    for (
        ...
        output_binary
    ):  # binary search results that did not meet other vague requirements
    ...
    outputProducts_vaguenessGreaterZero = s_p.sort_by_price(
    ...
    c_i_helper.add_matched_information(
        data, outputProducts_vaguenessGreaterZero, allDocs
    )
    print("after matched Info ..")

    output_produts_asin_keys = [product['asin'] for product in outputProducts_vaguenessGreaterZero]
    reviews_data = get_reviews_data(output_produts_asin_keys)
    # Needed in frontend
    outputProducts_vaguenessGreaterZero_with_original_query = [
        outputProducts_vaguenessGreaterZero,
        data,
        reviews_data
    ]
...
def get_vague_and_binary_lists(clean_data1):
    ...
    for field in clean_data1.keys():
        if clean_data1[field]["weight"] == 5:
            # bool_search_default = True
            binary_clean_data[field] = clean_data1[
                field
            ]  # weigth doesn't matter for boolean search
        else:
    ...
    for fieldName in fieldNameToValueDict:
        ...
        if range_field_name in fieldNameToValueDict[fieldName]:
            result["range"].update(
                {
                    fieldName: {
                        "range": fieldNameToValueDict[fieldName][range_field_name],
        ...
        # In case of multiple values for the same field, example : ram : [2,4,6], ram is either 2, 4 or 6.
            ...
            if (
                type(fieldNameToValueDict[fieldName][value_field_name]) is list
                and len(fieldNameToValueDict[fieldName][value_field_name]) > 0
            ):
                if type(fieldNameToValueDict[fieldName][value_field_name][0]) is str:
                    fieldNameToValueDict[fieldName][value_field_name] = [
                        x.lower()
                        for x in fieldNameToValueDict[fieldName][value_field_name]
                    ]
                    result["binary"].update(
                        {
                            fieldName: {
                                "value": fieldNameToValueDict[fieldName][
                                    value_field_name
                                ],
                                "weight": fieldNameToValueDict[fieldName]["weight"],
                            }
                ...
                # --------------------------------------------------------------------------------------------------------------------------------#
                    ...
                    type(fieldNameToValueDict[fieldName][value_field_name][0]) is int
                    or type(fieldNameToValueDict[fieldName][value_field_name][0])
                    is float
                ):
                    result["vague"].update(
                        {
        ...
        elif type(fieldNameToValueDict[fieldName]) is dict and (
            "intent" in fieldNameToValueDict[fieldName]
        ):
            ...
            result["alexa"].update(
                {
                    fieldName: {
                        "intent": fieldNameToValueDict[fieldName]["intent"],
        ...
        for field_name in field_value_dict[field_type]:
            if field_name != "price" and field_name != "hardDriveSize":
                ...
                if field_type is "binary":
                    ...
                    res_search.append(
                        binary_searcher.compute_binary_text(
                            field_name, field_weight, field_value
                        )
                    )
            ...
            with default implementation of vague function and membership function calculations."""
                    if len(field_value_dict[field_type][field_name]["range"]) == 1:
                        for range in field_value_dict[field_type][field_name]["range"]:
                            ...
                            if "minValue" in range and "maxValue" in range:
                                ...
                                if "counter" in range:
                                    ...
                                    res_search.append(
                                        range_searcher.compute_vague_range(
                                            allDocs,
                                            field_name,
                                            field_weight,
                                            min_value,
                        ...
                        res_search.append(
                            range_searcher.compute_vague_range_mult_intervals(
                                allDocs,
                                field_name,
                                field_weight,
                                field_value_dict[field_type][field_name]["range"],
                ...
                # Values for binary key in the dict, these will be searched in the value_searcher
                    ...
                    field_value = field_value_dict[field_type][field_name]["value"]
                    res_search.append(
                        value_searcher.compute_vague_value(
                            allDocs, field_name, field_weight, field_value
                        )
                    )
                # --------------------------------------------------------------------#
                # Values for alexa
...
def get_alexa_search_result(allDocs, field_value_dict, alexa_searcher):
    ...
    for field_name in field_value_dict:
        ...
        res_search.append(
            alexa_searcher.compute_boolean_value(
                field_name, 6, field_value, field_intent
            )
        )
