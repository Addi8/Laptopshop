...
class ColorInformation:
        ...
        for laptop in laptops:
            ...
            for field in laptop:
                if laptop[field] is not None:
                    if (field == "hddSize" or field == "ssdSize") and (
                        laptop[field] > 0
                    ):
                        if "hardDriveSize" in query:
                            if len(query["hardDriveSize"]["hardDriveSizeRange"]) == 1:
                                color_value = self.get_ranged_field_color(
                                    laptop[field],
                                    query["hardDriveSize"]["hardDriveSizeRange"],
                                    all_values_dict,
                                    field,
                                )
                                ...
                                colors = []
                                for interval_range in query["hardDriveSize"][
                                    "hardDriveSizeRange"
                                ]:
                                    color_value = self.get_ranged_field_color(
                                        laptop[field],
                        ...
                        if field_range_name in query[field]:
                            ...
                            if (
                                len(query[field][field_value_name]) > 0
                                and type(query[field][field_value_name][0]) is str
                            ):
                                color_value = self.get_text_value_field_color(
                                    laptop[field],
                                    query[field][field_value_name],
                                    allDocs,
                                    field,
                                )
                                result.update({field: color_value})
                            else:
                                color_value = self.get_discrete_value_field_color(
                                    laptop[field],
                                    query[field][field_value_name],
                                    all_values_dict,
                                    field,
    ...
    def get_text_value_field_color(
        self, laptop_value, query_values, allDocs, field_name
    ):
    ...
    def get_discrete_value_field_color(
        self, laptop_value, query_values, all_values_dict, field_name
    ):
        ...
        for value in query_values:
            ...
            if (
                float(laptop_value) >= lowerSupport
                and float(laptop_value) <= upperSupport
            ):
                return "yellow"

        return "red"

    def get_ranged_field_color(
        self, laptop_value, query_values, all_values_dict, field_name
    ):
        ...
        for value in query_values:
            ...
            if "minValue" in value and "maxValue" in value:
                if (
                    float(laptop_value) >= value["minValue"]
                    and float(laptop_value) <= value["maxValue"]
                ):
                    return "green"
        ...
        for value in query_values:
            ...
            if "counter" in query_values[0]:
                ...
                trapezoid_wing_size = (interval / query_values[0]["counter"]) * (
                    1 / query_values[0]["counter"]
                )
    ...
    def prozessThreshholdPrice(self, searchedValues):
        threshhold = 0
        try:
            threshhold = float(searchedValues["maxValue"]) - float(
                searchedValues["minValue"]
            )
        except:
            threshhold = 50
        if threshhold > 50:
