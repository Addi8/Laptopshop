...
def searchBinary():
    ...
    res = es.search(index="laptop", body=query)
...
def alexa_search():
    ...
    intent_variable_value = data[data["intentVariable"]][
        data["intentVariable"] + "Value"
    ]
...
def getElementByAsin(asin):
    product = es.search(
        index="laptops", body={"query": {"match": {"asin": asin}}, "size": 1}
    )
...
def getSample():
    allDocs = es.search(
        index="laptops", body={"query": {"match": {"avgRating": 5}}, "size": 10}
    )
